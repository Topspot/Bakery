...
if ( defined('WP_DEBUG') && WP_DEBUG ){
	...
	$the_core_wp_debug_mode_description_text = '<span class="fw-error-message">' .__( 'Displays whether or not WordPress is in Debug Mode. This mode is used by developers to test the theme. We recommend you turn it off for an optimal user experience on your website.', 'the-core' ).' <a href="https://codex.wordpress.org/WP_DEBUG" target="_blank">'.__('Learn how to do it', 'the-core').'</a></span>';
}
else{
...
if ( the_core_return_memory_size( ini_get('post_max_size') ) < $the_core_requirements_post_max_size ) {
	...
	$the_core_php_post_max_size_description_text = '<span class="fw-error-message">' .esc_html__( 'The largest file size that can be contained in one post.', 'the-core'  ).' '. esc_html__( 'We recommend setting the post maximum size to at least:', 'the-core' ) .' <strong>'.size_format($the_core_requirements_post_max_size). '</strong>'.'. <a href="http://docs.themefuse.com/the-core/your-theme/theme-settings/how-to-set-a-maximum-file-upload-size" target="_blank">'.__('Learn how to do it','the-core').'</a></span>';
}
else{
...
if ( $the_core_time_limit < $the_core_required_php_time_limit && $the_core_time_limit != 0 ) {
	...
	$the_core_php_time_limit_description_text = '<span class="fw-error-message">'.esc_html__( 'The amount of time (in seconds) that your site will spend on a single operation before timing out (to avoid server lockups).', 'the-core'  ).' '.__( 'We recommend setting the maximum execution time to at least', 'the-core').' <strong>'.$the_core_required_php_time_limit.'</strong>'.'. <a href="http://codex.wordpress.org/Common_WordPress_Errors#Maximum_execution_time_exceeded" target="_blank">'.__('Learn how to do it','the-core').'</a></span>';
} else {
...
if ( $the_core_max_input_vars < $the_core_required_input_vars ) {
	$the_core_php_max_input_vars_description_text = '<span class="fw-error-message">'.esc_html__( 'The maximum number of variables your server can use for a single function to avoid overloads.', 'the-core'  ). ' '.__( 'Please increase the maximum input variables limit to:','the-core').' <strong>' . $the_core_required_input_vars . '</strong>'.'. <a href="http://docs.themefuse.com/the-core/your-theme/theme-settings/how-to-increase-the-maximum-input-variables-limit" target="_blank">'.__('Learn how to do it','the-core').'</a></span>';
...
if ( wp_max_upload_size() < $the_core_requirements_max_upload_size ) {
	...
	$the_core_max_upload_size_description_text = '<span class="fw-error-message">' . esc_attr__( 'The largest file size that can be uploaded to your WordPress installation.', 'the-core'  ). ' '.esc_html__( 'We recommend setting the maximum upload file size to at least:', 'the-core' ) .' <strong>'.size_format($the_core_requirements_max_upload_size). '</strong>. <a href="http://docs.themefuse.com/the-core/your-theme/theme-settings/how-to-set-a-maximum-file-upload-size" target="_blank">'.__('Learn how to do it', 'the-core').'</a></span>';
}
else{
