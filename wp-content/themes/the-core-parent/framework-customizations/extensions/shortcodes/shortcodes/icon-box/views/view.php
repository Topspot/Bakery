...
// get icon position
$icon_position = $icon_type_picker == 'fw-iconbox-3' ? $atts['icon_type_picker']['fw-iconbox-3']['icon-position'] : '';
...
if( $icon_img_class == 'fw-iconbox-image-type'){
	...
	if( isset($atts['icon_type']['upload-icon']['icon_size']) && !empty($atts['icon_type']['upload-icon']['icon_size']) ){
		$custom_image_style = ' width: '.$size.'px; height:'.$size.'px;';
	}
	if( $icon_type_picker == 'fw-iconbox-2'){
		$margin = 'margin-left';
		if( is_rtl() ){
			$margin = 'margin-right';
		}
		$text_style = 'style="'.$margin.': '.($size+20).'px;"';
	}
}
else{
	if( isset($atts['icon_type']['icon-class']['icon_class']) && !empty($atts['icon_type']['icon-class']['icon_class']) ){
		$size = ((int)$atts['icon_type']['icon-class']['icon_size']+20);
		$custom_image_style = ' width: '.$size.'px; height:'.$size.'px; line-height: '.( (int)$size+1 ).'px; font-size:' . (int) $icon_type['icon-class']['icon_size'] . 'px;';
		$margin = 'margin-left';
		if( is_rtl() ){
			$margin = 'margin-right';
		}
		if( $icon_type_picker == 'fw-iconbox-2' && $icon_bg_class != 'bg-on' ){
			$text_style = 'style="'.$margin.': '.($size+10).'px;"';
		}
		elseif( $icon_type_picker == 'fw-iconbox-2'){
			$text_style = 'style="'.$margin.': '.($size+20).'px;"';
		}
	}
}
...
<div class="fw-iconbox clearfix tf-sh-<?php echo esc_attr( $atts['unique_id'] ); ?> <?php echo esc_attr( $icon_type_picker ) . ' ' . esc_attr( $icon_position ) . ' ' . esc_attr( $icon_img_class ) . ' ' . esc_attr( $icon_bg_class ) . ' ' . esc_attr( $icon_bg_circle ) . ' ' . esc_attr( $atts['class'] ); ?>" <?php echo $data_animation; ?>>
	<div class="fw-iconbox-image <?php echo the_core_translate( esc_attr( $extra_class ) ); ?>" style="<?php echo $icon_bg_color, $custom_image_style; ?>">
	...
	<div class="fw-iconbox-aside" <?php echo $text_style; ?>>
		<?php if ( ! empty( $title ) ) : ?>
