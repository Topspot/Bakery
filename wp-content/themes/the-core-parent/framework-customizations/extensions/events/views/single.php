...
?>
<section class="fw-main-row <?php the_core_get_content_class( 'main', $the_core_sidebar_position ); ?>" role="main" itemprop="mainEntity" itemscope="itemscope" itemtype="http://schema.org/Event">
						...
						<article id="post-<?php the_ID(); ?>" <?php post_class( "post post-details event-details" ); ?>>
								...
								<?php foreach ( $options['event_children'] as $key => $row ) : ?>
									...
									<ul class="details-event">
										<li itemprop="startDate">
											<b><?php esc_html_e( 'Start', 'the-core' ) ?>:</b> <?php echo date( $final_date_format, strtotime( $row['event_date_range']['from'] ) ); ?><?php if( $dates_count >= 2 && $the_core_count == 1) { echo '<span class="fw-events-more-link">, '.'<a class="fw-show-more-events closed" hre="#">'.(count($options['event_children'])-1).' '; esc_html_e('More', 'the-core'); echo '</a></span>'; } ?>
										</li>
										<li><b><?php esc_html_e( 'End', 'the-core' ) ?>:</b> <?php echo date( $final_date_format, strtotime( $row['event_date_range']['to'] ) ); ?></li>
							...
							<div class="fw-row fw-location-info">
								<?php if( !empty($options['event_location']) ) : ?>
									<div class="fw-col-sm-7 fw-event-location" itemprop="location" itemscope itemtype="http://schema.org/Place">
										...
										<span itemprop="address" itemscope itemtype="http://schema.org/PostalAddress">
											<?php if( !empty($options['event_location']['state']) ) : ?>
												<span itemprop="addressLocality"><?php if( $options['event_location']['city'] != $options['event_location']['state'] && !empty($options['event_location']['city']) ) echo $options['event_location']['city'].', '; ?><?php echo esc_attr($options['event_location']['state']); ?></span><?php if( !empty($options['event_location']['country']) ) : ?>,<?php endif; ?>
											<?php endif; ?>
								...
								<div class="fw-col-sm-5 fw-event-offers" itemprop="offers" itemscope itemtype="http://schema.org/Offer">
									<?php if( !empty($event_options['ticket_price']) ) : ?>
										<span class="fw-event-price" itemprop="price" content="<?php echo (int)$event_options['ticket_price']; ?>"><?php esc_html_e('Price:', 'the-core'); ?> <span class="ticket-price-symbol"><?php esc_html_e('$', 'the-core'); ?></span><?php echo esc_attr($event_options['ticket_price']); ?></span>
									<?php endif; ?>
									<?php if( !empty($event_options['ticket_link']) ) : ?>
										<span itemprop="priceCurrency" content="<?php esc_html_e('USD', 'the-core'); ?>">
