...
if ( ! function_exists( '_the_core_action_theme_setup' ) ) :
	...
	function _the_core_action_theme_setup() {
		...
		add_theme_support( 'post-thumbnails', array( 'post', 'fw-portfolio', 'fw-event', 'fw-learning-articles', 'fw-learning-courses', 'product', 'wpsc-product' ) );
...
if ( ! function_exists( '_the_core_filter_theme_wp_title' ) ) :
	/**
	 ...
	 * @internal
	 */
	function _the_core_filter_theme_wp_title( $title, $sep ) {
		global $paged, $page;
		...
		if ( $paged >= 2 || $page >= 2 ) {
			$title = "$title $sep " . sprintf( esc_html__( 'Page %s', 'the-core' ), max( $paged, $page ) );
		}

		return $title;
	}
endif;
add_filter( 'wp_title', '_the_core_filter_theme_wp_title', 10, 2 );
...
if ( ! function_exists( '_the_core_filter_settings_form_ajax_save_loading_extra_message' ) ) :
	/**
	 * Show a message when theme settings are saved
	 */
	function _the_core_filter_settings_form_ajax_save_loading_extra_message() {
		return '<a href="http://docs.themefuse.com/the-core/your-theme/theme-settings/saving-the-general-settings#saving" target="_blank">'.esc_html__('Takes too long? Find out why.', 'the-core').'</a>';
	}
endif;
add_filter( 'fw_settings_form_ajax_save_loading_extra_message', '_the_core_filter_settings_form_ajax_save_loading_extra_message' );
...
/** proces less files and generate the-core-style.css in uploads folder */
{
	/**
	 ...
	 */
    if ( ! function_exists( 'the_core_enqueue_less' ) ) :
        function the_core_enqueue_less( $less_file = '', $extension_name = 'theme' ) {
            global $fw_less_files;
            $file = preg_replace( '/[\s\W]+/', '_', $less_file );
            if ( empty( $extension_name ) || 'theme' == $extension_name ) {
                $fw_less_files["theme_{$file}"] = '@import "' . locate_template( 'styles-less/' . $less_file ) . '";';

                return;
            }
            if ( $extension = fw_ext( $extension_name ) ) {
                $fw_less_files["{$extension_name}_{$file}"] = '@import "' . $extension->locate_path( $less_file ) . '";';
            }
        }
    endif;
    ...
    if ( ! function_exists( '_the_core_action_settings_form_saved' ) ) :
        function _the_core_action_settings_form_saved() {
            FW_Session::set( 'fw-less-files/options_saved', true );
            _the_core_action_theme_generate_styles();
        }
    endif;
	add_action( 'fw_settings_form_saved', '_the_core_action_settings_form_saved', 999, 2 );

	function _the_core_action_theme_generate_styles() {
		global $fw_less_files, $fw_stylesheet_directory, $fw_template_directory;

		$fw_stylesheet_directory   = get_stylesheet();
		$fw_template_directory     = get_template();
		$the_core_template_directory_uri = get_template_directory_uri();
		$upload_dir                = wp_upload_dir();
		$style_dir                 = $upload_dir['basedir'];

		if ( defined( 'FW' ) ) {
			if(class_exists('FW_Session')) {
				$options_saved = FW_Session::get( 'fw-less-files/options_saved' );
			}
			else{
				$options_saved = false;
			}
		...
		/**
		 * Less files are generated in one file the-core-style.css, in upload folder, after options are saved or if
		 * the-core-style.css file doesn't exist yet
		 */
		if ( ! file_exists( $style_dir . '/the-core-style.css' ) || $options_saved ) {
			$errors     = array();
			$style_less = locate_template( 'styles-less/fw-theme.less' );
			if( is_rtl() ) {
				$rtl_less = locate_template( 'styles-less/fw-theme/rtl/rtl.less' );
			}

			if ( defined( 'FW' ) && !class_exists('Less_Parser') ) {
				$errors[] = sprintf( esc_html__( 'Less processor not found. Install %s Less & scss PHP Compilers plugin%s, or activate %s the plugin %s', 'the-core' ), '<a href="' . esc_url( admin_url() ) . 'plugin-install.php?tab=plugin-information&plugin=lessphp&TB_iframe=true&width=772&height=505' . '">', '</a>' , '<a href="' . esc_url( admin_url() ) . 'plugins.php' . '">', '</a>' );
			}

			if ( ! $style_less ) {
				$errors[] = sprintf( esc_html__( 'Required file not found:  %s', 'the-core' ), '/styles-less/fw-theme.less' );
			}
			...
			if ( ! empty( $errors ) ) {
				if ( defined( 'FW' ) ) {
					FW_Flash_Messages::add(
						'fw_settings_less_not_found',
						implode( "<br />\n", $errors ),
						'error' );
				} else {
					/**
					 * backward compatibility if unyson isn't installed.
					 * this function _the_core_action_theme_generate_styles() is hooked to admin_notices action
					 */
					?>
					<div class="error">
			...
			try {
				...
				$directories = array( get_stylesheet_directory() . '/styles-less/' => '/' );
				...
				if ( ! function_exists( 'verify_less_in_child_theme' ) ) {
					function verify_less_in_child_theme( $evald ) {
						global $fw_stylesheet_directory, $fw_template_directory;

						$evaldPath = $evald->PathAndUri();
						$full_path = preg_replace( '/' . $fw_template_directory . '/', $fw_stylesheet_directory, $evaldPath[0] );

						if ( file_exists( $full_path ) ) {
							$uri = preg_replace( '/' . $fw_template_directory . '/', $fw_stylesheet_directory, $evaldPath[1] );
				...
				if ( defined( 'FW' ) && class_exists('Less_Parser') ) {
					...
					$parser->parseFile( $style_less, $the_core_template_directory_uri );
					if ( is_rtl() ) {
						$parser->parseFile( $rtl_less, $the_core_template_directory_uri );
					}

					/** $fw_less_files is an array of less files path included from extensions with the_core_enqueue_less() */
					if ( is_array( $fw_less_files ) ) {
						$style_less_extensions = implode( ' ', $fw_less_files );
						$parser->parse( $style_less_extensions );
					}

					/** add at the end custom.less file which may contain user customizations */
					if ( $custom_less = locate_template( 'styles-less/custom.less' ) ) {
						$parser->parseFile( $custom_less, $the_core_template_directory_uri );
					}
					...
					//$quick_css .= $blog_button_styles.$the_core_blog_title_styles;
					if ( $quick_css ) {
						$parser->parse( $quick_css );
					}

					/**
					 * get values from admin styling and overwrite them when processing less files
					 */
					$the_core_modified_variables = array();
					include( locate_template( 'theme-includes/admin-styling.php' ) );
					$parser->ModifyVars( $the_core_modified_variables );

					/**
					 * Parse all css and write generated styles to the-core-style.css in upload folder,
					...
					if ( ! $wp_filesystem->put_contents( $style_dir . '/the-core-style.css', $css ) ) {
						esc_html_e( "error write content", "the-core" );
					}
				}
				...
				if ( defined( 'FW' ) ) {
					FW_Flash_Messages::add(
						'fw_settings_less_error',
						sprintf( esc_html__( 'lessphp fatal error:  %s', 'the-core' ), $e->getMessage() ),
						'error' );
				} else {
					/**
					 * backward compatibility if unyson isn't installed.
					...
					?>
					<div class="error">
						<p><?php printf( esc_html__( 'lessphp fatal error:  %s', 'the-core' ), $e->getMessage() ); ?></p>
					</div>
				<?php
				}
			}

			/** delete path from session for less files from extensions, after we parse them */
			if ( defined( 'FW' ) ) {
				FW_Session::del( 'fw-less-files' );
			}

			/** show parsed files in admin on save options for debugging */
			if ( WP_DEBUG && defined( 'FW' ) && defined( 'FW_PARSED_LESS' ) ) {
				$imported_files = $parser->allParsedFiles();
				if ( defined( 'FW' ) ) {
					FW_Flash_Messages::add(
						'fw_settings_less_debug',
						esc_html__( 'less files parsed', 'the-core' ) . ':<br />' . implode( '<br>', $imported_files ),
						'info' );
				}
			}
		}
	...
	add_action( 'admin_print_footer_scripts', '_the_core_action_theme_generate_styles', 99 );
}
...
add_action( 'wp_print_styles', '_the_core_action_print_google_fonts_link' );
...
if ( ! function_exists( '_the_core_filter_body_class' ) ) :
	...
	function _the_core_filter_body_class( $classes ) {
		...
		if ( function_exists( 'fw_get_db_settings_option' ) ) {
			...
			if ( isset( $the_core_general_settings['header_settings']['enable_absolute_header']['selected_value'] ) ) {
			...
			if ( isset( $the_core_general_settings['header_settings']['enable_sticky_header'] ) ) {
			...
			elseif ( $the_core_general_settings['logo_settings']['logo']['selected_value'] == 'text' ) {
...
if ( ! function_exists( '_the_core_action_shortcode_accordion_enqueue_dynamic_css' ) ):
	...
	function _the_core_action_shortcode_accordion_enqueue_dynamic_css( $data ) {
		...
		if ( isset( $atts['advanced_styling'] ) ) {
			...
			if( !empty( $text_advanced_styling['styles'] ) ) $final_styles .= '.tf-sh-' . $atts['unique_id'] . ' .panel-body, .tf-sh-' . $atts['unique_id'] . ' .panel-body * {'.$text_advanced_styling['styles'].'}';
	...
	function _the_core_action_shortcode_team_member_enqueue_dynamic_css( $data ) {
		...
		if ( isset( $atts['content_bg_color'] ) ) {
			...
			if( !empty($content_bg_color['color']) ) {
				$final_styles .= '.tf-sh-' . $atts['unique_id'] . ' { background-color: '.$content_bg_color['color'].' }';
			}
		}
...
if ( ! function_exists( '_the_core_action_shortcode_table_enqueue_dynamic_css' ) ):
	...
	function _the_core_action_shortcode_table_enqueue_dynamic_css( $data ) {
		...
		// for table buttons styling
		if(isset($atts['table']['content']) && !empty($atts['table']['content']))
		{
			foreach($atts['table']['content'] as $table){
				if(!empty($table)) {
					foreach ($table as $btn) {
						if(isset($btn['button']))
						{
							if( isset($btn['button']['style']['selected']) ){
								if($btn['button']['style']['selected'] == 'fw-btn-3'){
									if( isset($btn['button']['style']['fw-btn-3']['border_size']) && !empty($btn['button']['style']['fw-btn-3']['border_size']) ){
										$final_styles .= $id. ' .tf-sh-' . $btn['button']['unique_id'] . '{ border-top-width: '.(int)$btn['button']['style']['fw-btn-3']['border_size'].'px; border-bottom-width: '.(int)$btn['button']['style']['fw-btn-3']['border_size'].'px; }';
									}
									// btn color
									if ( isset( $btn['button']['normal_color'] ) ) {
										$normal_color = the_core_get_color_palette_color_and_class( $btn['button']['normal_color'], array('return_color' => true ) );
										if( !empty($normal_color['color']) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . '{ border-bottom-color: '.$normal_color['color'].'; border-top-color: '.$normal_color['color'].' }';
									}
									// hover color
									if ( isset( $btn['button']['hover_color'] ) ) {
										$hover_color = the_core_get_color_palette_color_and_class( $btn['button']['hover_color'], array('return_color' => true ) );
										if( !empty($hover_color['color']) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . ':hover { background-color: '.$hover_color['color'].'; border-bottom-color: '.$hover_color['color'].'; border-top-color: '.$hover_color['color'].' }';
									}
								}
								elseif($btn['button']['style']['selected'] == 'fw-btn-2'){
									if( isset($btn['button']['style']['fw-btn-2']['border_size']) && !empty($btn['button']['style']['fw-btn-2']['border_size']) ){
										$final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . '{ border-width: '.(int)$btn['button']['style']['fw-btn-2']['border_size'].'px; }';
									}
									if( isset($btn['button']['style']['fw-btn-2']['border_radius']) && !empty($btn['button']['style']['fw-btn-2']['border_radius']) ){
										$final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . '{ border-radius: '.(int)$btn['button']['style']['fw-btn-2']['border_radius'].'px; }';
									}

									// btn color
									if ( isset( $btn['button']['normal_color'] ) ) {
										$normal_color = the_core_get_color_palette_color_and_class( $btn['button']['normal_color'], array('return_color' => true ) );
										if( !empty($normal_color['color']) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . '{ border-color: '.$normal_color['color'].' }';
									}
									// hover color
									if ( isset( $btn['button']['hover_color'] ) ) {
										$hover_color = the_core_get_color_palette_color_and_class( $btn['button']['hover_color'], array('return_color' => true ) );
										if( !empty($hover_color['color']) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . ':hover { background-color: '.$hover_color['color'].'; border-color: '.$hover_color['color'].' }';
									}
								}
								elseif( $btn['button']['style']['selected'] == 'fw-btn-1' ) {
									if( isset($btn['button']['style']['fw-btn-1']['border_radius']) && !empty($btn['button']['style']['fw-btn-1']['border_radius']) ){
										$final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . '{ border-radius: '.(int)$btn['button']['style']['fw-btn-1']['border_radius'].'px; }';
									}

									// btn color
									if ( isset( $btn['button']['normal_color'] ) ) {
										$normal_color = the_core_get_color_palette_color_and_class( $btn['button']['normal_color'], array('return_color' => true ) );
										if( !empty($normal_color['color']) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . '{ background-color: '.$normal_color['color'].' }';
									}
									// hover color
									if ( isset( $btn['button']['hover_color'] ) ) {
										$hover_color = the_core_get_color_palette_color_and_class( $btn['button']['hover_color'], array('return_color' => true ) );
										if( !empty($hover_color['color']) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . ':hover { background-color: '.$hover_color['color'].' }';
									}
							...
							// label advanced styling
							if ( isset( $btn['button']['label_advanced_styling'] ) ) {
								// text advanced styling
								$text_advanced_styling  = the_core_get_shortcode_advanced_styles( $btn['button']['label_advanced_styling']['text'], array('return_color' => true ) );
								if( !empty( $text_advanced_styling['styles'] ) ) $final_styles .= $id.' .tf-sh-' . $btn['button']['unique_id'] . ' span {'.$text_advanced_styling['styles'].'}';
								// responsive text styling
								$text_responsive_styles = the_core_responsive_heading_styles( array( 'styles' => $btn['button']['label_advanced_styling']['text'], 'selector' => $id.' .tf-sh-' . $btn['button']['unique_id'] . ' span' ) );
								if( !empty($text_responsive_styles) ) $final_styles .= '@media(max-width:767px){'.$text_responsive_styles.'}';

								// hover text color
								if ( isset( $btn['button']['label_advanced_styling']['hover_text_color'] ) ) {
									$hover_text_color = the_core_get_color_palette_color_and_class( $btn['button']['label_advanced_styling']['hover_text_color'], array('return_color' => true ) );
									if( !empty($hover_text_color['color']) ) $final_styles .= $id. ' .tf-sh-' . $btn['button']['unique_id'].':hover span' . '{ color: '.$hover_text_color['color'].' }';
								}
							}

							if (empty($final_styles)) continue;
...
if ( ! function_exists( '_the_core_action_shortcode_media_video_enqueue_dynamic_css' ) ):
	...
	function _the_core_action_shortcode_media_video_enqueue_dynamic_css( $data ) {
		...
		if( isset($atts['frame']['selected']) && $atts['frame']['selected'] == 'yes' ){
			$final_styles .= '.tf-sh-' . $atts['unique_id'] . ' iframe { border-style: solid;';
	...
	function _the_core_action_shortcode_portfolio_enqueue_dynamic_css( $data ) {
		...
		elseif($atts['portfolio_style']['selected'] == 'style2') {
			...
			if (isset($portfolio['style2_advanced_styling'])) {
				...
				//get paddings
				$padding_top = (isset($portfolio_settings['padding_top']) && !empty($portfolio_settings['padding_top'])) ? 'padding-top:'.(int)esc_attr($portfolio_settings['padding_top']).'px;' : '';
				$padding_right = (isset($portfolio_settings['padding_right']) && !empty($portfolio_settings['padding_right'])) ? 'padding-right:'.(int)esc_attr($portfolio_settings['padding_right']).'px;' : '';
				$padding_bottom = (isset($portfolio_settings['padding_bottom']) && !empty($portfolio_settings['padding_bottom'])) ? 'padding-bottom:'.(int)esc_attr($portfolio_settings['padding_bottom']).'px;' : '';
				$padding_left = (isset($portfolio_settings['padding_left']) && !empty($portfolio_settings['padding_left'])) ? 'padding-left:'.(int)esc_attr($portfolio_settings['padding_left']).'px;' : '';
...
add_filter('fw_slider_add_shortcode_extra_data', '_the_core_action_slider_shortcode_extra_data', 10,2);
...
add_action( 'wp_head', '_the_core_action_print_category_special_styles' );
...
function _the_core_filter_fw_ext_backups_demos($demos) {
	$demos_array = array(
        ...
        'the-pathfinder' => array(
            ...
            'preview_link' => 'http://demo.themefuse.com/the-pathfinder/',
        ),
	);
...
if( !function_exists('_the_core_filter_update_footer') ):
    ...
    function _the_core_filter_update_footer( $html ) {
        $the_core_version = defined( 'FW' ) ? fw()->theme->manifest->get_version() : '1.0';

        $html .= ' | <a href="//themefuse.com/changelogs/the-core/" target="_blank">'.esc_html__('Theme', 'fw').'. '.$the_core_version.'</a>';
        return $html;
    }
endif;
