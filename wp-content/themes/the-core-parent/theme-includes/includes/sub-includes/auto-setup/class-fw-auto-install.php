...
class FW_Admin_Auto_Install extends FW_Simple_Auto_Install {
	...
	public function activate_demo_plugins() {
		...
		$response = FW_Installer_Helper::activate_plugins( $_POST['demo_plugins'] );
		...
		if ( ! empty( $failed_plugins ) ) {
			wp_send_json_error( array(
				'message' => sprintf( esc_html__( 'Failed to activate required plugins. %s', 'the-core' ), implode( ', ', $failed_plugins ) ),
			) );
		}
	...
	public function activate_unyson() {
		...
		$response = FW_Installer_Helper::activate_plugin( array(
			'name' => 'Unyson',
			'slug' => 'unyson'
		) );
	...
	public function activate_required_plugins() {
		...
		$response = FW_Installer_Helper::activate_plugins( $this->get_required_plugins() );
		...
		if ( ! empty( $failed_plugins ) ) {
			$this->insert_step_status( 'activate-required-plugins', false, $message );
		...
		$this->insert_step_status( 'activate-required-plugins', true, $message );
...
class FW_Super_Admin_Auto_Install extends FW_Admin_Auto_Install {
	...
	public function install_demo_plugins() {
		...
		$response = FW_Installer_Helper::install_plugins( $_POST['demo_plugins'] );
	...
	public function install_unyson() {
		...
		$response = FW_Installer_Helper::install_plugin( array(
			'name' => 'Unyson',
			'slug' => 'unyson'
		) );
	...
	public function install_required_plugins() {
		...
		$response = FW_Installer_Helper::install_plugins( $this->get_required_plugins() );
	...
	public function install_child_theme() {
		...
		if ( ! $wp_filesystem->is_dir( WP_CONTENT_DIR . '/themes/' . $this->get_theme_id() . '-child' ) ) {
			$response = FW_Installer_Helper::download_and_install_a_package( $this->child_theme_source, WP_CONTENT_DIR . '/themes', array(
				'type'   => 'theme',
				'action' => 'install',
...
class FW_Installer_Helper {
	...
	public static function activate_plugins( $plugins ) {
		...
		foreach ( $plugins as $plugin ) {
			$status[ $plugin['slug'] ]['activate'] = FW_Installer_Helper::activate_plugin( $plugin );
		}

		return $status;
	}

	private static function plugins_api( $action, $args = null ) {
	...
	public static function install_plugin( $plugin ) {
		...
		// For pre-packaged and self-hosted
		if ( empty( $plugin['source'] ) ) {
			$call_api = FW_Installer_Helper::plugins_api( 'plugin_information', array( 'slug' => $plugin['slug'] ) );
			if ( is_wp_error( $call_api ) ) {
				return array(
					'success' => false,
					'data'    => array(
						'message' => $call_api->get_error_message()
					)
				);
			}
			$plugin['source'] = $call_api->download_link;
		}

		//Update installed plugins

		if ( ( $installed = get_plugins( '/' . $plugin['slug'] ) ) && ( $plugin_file = array_keys( $installed ) ) && ! empty( $installed[ $plugin_file[0] ]['Version'] ) ) {
			//Plugin is installed
			if ( ! empty( $plugin['version'] ) ) {
				if ( version_compare( $installed[ $plugin_file[0] ]['Version'], $plugin['version'], '<' ) ) {
					$hooks['action'] = 'update';
					$hooks['plugin'] = $plugin['slug'] . '/' . $plugin_file[0];
				} else {
					return array(
						'success' => true,
						'data'    => array(
		...
		$response = self::download_and_install_a_package( $plugin['source'], WP_PLUGIN_DIR, $hooks );
